    ...
    protected function _parse_put()
    {
        if ($this->request->format) {
            ...
            if ($this->request->format === 'json') {
                $this->_put_args = json_decode($this->input->raw_input_stream, true);
            }
    ...
    public function post($key = null, $xss_clean = null)
    {
        if ($key === null) {
            foreach (new RecursiveIteratorIterator(new RecursiveArrayIterator($this->_post_args), RecursiveIteratorIterator::CATCH_GET_CHILD) as $key => $value) {
                return $this->security->xss_clean($this->_post_args);
            }
    ...
    protected function _prepare_basic_auth()
    {
        ...
        if ($username !== null) {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0) {
                ...
                list($username, $password) = explode(':', base64_decode(), 2);
            }
        }
